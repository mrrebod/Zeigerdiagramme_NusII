...
\usepackage{tikz}
\usetikzlibrary{angles, calc, positioning, intersections, topaths, decorations, shapes, arrows.meta}
...
\begin{flushright}
	...
	\begin{tikzpicture}
		...
		\draw [name intersections={of=i2 and xAchse, name=interi2}, very thin]
			(interi2-2) -- ++(-90:0.5cm) node(HNode2)[HelpNode]{};	%Plotted Hilfslinie und genereirt Hilfsnode am Ende	
...
\begin{frame}\frametitle{Serien vs. Parallelschwingkreis I}	
	\begin{flushright}
		%-----------------------------------------------------------------------------------------------------------------%
		\tikzsetnextfilename{SerienVsParallelschwingkreis_I}
		\begin{tikzpicture}[scale=0.6]
			%%% Definitionen des Settings - !!! ACHTUNG: Gleich wie in Zeigerdiagramme für Serie-/Parallelschwingkreis !!! %%%
			\pgfkeys{/pgf/fpu=true,/pgf/fpu/output format=fixed}
			\def\Alpha{0}
			\def\FarbeSpannung{blue}
			\def\FarbeStrom{red}
			...
			%Differenz zur Resonanzfrequenze. Selbe für Serie und Parallel 
			\def\fre{\fres+3000}
			
			%Positionen der Plots definieren
			\def\Xstep{4.5} %Schritt in x-Richtung bis zum Nächsten Plot
			
			\def\Xoff{0} %X-Position für Serie 
			\def\Yoff{0} %Y-Position für Serie - gleich für Parallel
			...
			%Referenzpfeil
			\draw[-{Latex[length=2mm]}, color=\FarbeStrom,  thick]
			(\Xoff, \Yoff-\RefStep) -- (\Xoff+1.5,\Yoff-\RefStep) node[anchor= north] {$\mzeiger{i}\;$};
			
			% Widerstandsspannung
			\draw[-{Latex[length=2mm]}, color=\FarbeSpannung,  thick] 
				(\Alpha:0)++(\Xoff,\Yoff) -- ++(\Alpha:\uR) node [midway, below] {$\mzeiger{u}_R\;$}
			node[at end, inner sep=0pt, outer sep=0pt, minimum size=0pt](tmp) {};
			% Induktivitätsspannung
			\draw[-{Latex[length=2mm]}, color=\FarbeSpannung,  thick]
				(tmp) --++({\Alpha+90}:\uL) node [midway, left] {$\mzeiger{u}_L$} node[at end, inner sep=0pt, outer sep=0pt, minimum size=0pt](tmp) {};
			...
			% Eingangsspannung
			\draw[-{Latex[length=2mm]}, color=\FarbeSpannung,  thick] 
				(\Xoff,\Yoff) -- ++(tmp2) node [midway, above] {$\;\mzeiger{u}$};
			...
			%Referenzpfeil
			\draw[-{Latex[length=2mm]}, color=\FarbeSpannung,  thick]
				(\Xoff+\Xstep, \Yoff-\RefStep) -- (\Xoff+\Xstep+1.5,\Yoff-\RefStep) node[anchor= north] {$\mzeiger{u}\;$};	
			%  Widerstandsstrom
			\draw[-{Latex[length=2mm]}, color=\FarbeStrom, thick] 
			(\Alpha:0)++(\Xoff+\Xstep,\Yoff) -- ++(\Alpha:\iR) node [midway, below] {$\mzeiger{i}_R\;$}
				node[at end, inner sep=0pt, outer sep=0pt, minimum size=0pt](tmp) {};
			% Kondensatorstrom
			\draw[-{Latex[length=2mm]}, color=\FarbeStrom, thick]
				(tmp) --++({\Alpha+90}:\iC) node [midway, left] {$\mzeiger{i}_C$} node[at end, inner sep=0pt, outer sep=0pt, minimum size=0pt](tmp) {};
			...
			% Eingangsstrom
			\draw[-{Latex[length=2mm]}, color=\FarbeStrom, thick] 
				(\Xoff+\Xstep,\Yoff) -- ++(tmp2) node [midway, above] {$\;\mzeiger{i}$};
		\end{tikzpicture}
		%-----------------------------------------------------------------------------------------------------------------&
	\end{flushright}	
\end{frame}
...
\begin{frame}\frametitle{Asynchronmaschine - Ortskurve Statorstrom}	
	\begin{flushright}
		%-----------------------------------------------------------------------------------------------------------------%
		\tikzsetnextfilename{AsynchronmaschineOrtskurveStatorstrom}
		\begin{tikzpicture}
			%Verschiedene Styles definieren:
			[
			Achsenpfeil/.style={->},												  			%Style des Achsenpfeils
			HelpNode/.style={at end, inner sep=0pt, outer sep=0pt, minimum size=0pt},			%Options für Hilfs-Node (z.B. am Ende eines Zeigers)
			Zeiger/.style={-{Latex[length=2mm]}, thick},					          			%Style der komplexen Zeiger
			Winkelpfeil/.style={draw, {Latex[length=1mm]}-{Latex[length=1mm]}, angle radius = 9mm}, %Style der Winkelpfeil 
			HelpLine/.style={densely dashed, very thin}								 			%Hilfslinie
			]
			% Verschiedene Variabeln definieren:
			\def\USpitze{2.2};	  %Länge von Spannungszeiger
			\def\OssRad{3.0};	  %Radius des Ossanna Kreises
			
			% Definieren von verschiedenen Koordinaten
			\coordinate (Ursprung) at (0,0);		%Definieren des Ursprungs
			\coordinate (OssMitte) at (2.0cm,0.3cm);%Mittelpunkt des Ossanna Kreises
			\coordinate (xStart)   at (-0.1cm,0);  	%Startpunkt der x-Achse (wt-Achse)
			\coordinate (xEnd) 	   at (4.0cm,0);   	%Endpunkt der x-Achse (wt-Achse)
			\coordinate (yStart)   at (0cm,-1.5cm); %Startpunkt der y-Achse
			\coordinate (yEnd) 	   at (0cm,2.5cm);  %Endpunkt der y-Achse
			
			%%%%%%%%%%%%%%%%%%%%%%%%%% Zeigerdiagramm %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			%Achsen 
			\draw[Achsenpfeil] (xStart) -- (xEnd) node[at end, below]{$\operatorname{Im}$}; %x-Achse 
			\draw[Achsenpfeil] (yStart) -- (yEnd) node[at end, right] {$\operatorname{Re}$};%y-Achse
			
			%Ossanna Kreis  
			\node[circle, draw, minimum size=\OssRad cm, name path=Ossannakreis] (Ossanna) at (OssMitte) {};
			
			%Koordinaten von P_0, P_inf und P_A auf Ossannakreis
			\coordinate (P0) 	at (Ossanna.185);
			\coordinate (Pinf)  at (Ossanna.25);
			\coordinate (PA) 	at (Ossanna.50);
			
			%Mechanische Leistungslinie
			\draw[color=green!60!black, name path=MechLinie] (P0) -- (PA)
				node[above, scale=0.5, sloped, pos=0.7] {Mechanische Leistungslinie};
			%Drehmomentlinie
			\draw[color=green!60!black, name path=DrehLinie] (P0) -- (Pinf)
				node[below, scale=0.5, sloped, pos=0.6] {Drehmomentlinie}
				node [midway, scale=0.5, pos=0.8, black] {$\diagdown\!\!\diagdown$}; %Parallel line indicators
			%Parallele zu Drehmomentlinie
			\draw[color=green!60!black, name path=DrehlinieParallel] ([yshift=1.28cm]$(P0)!0.5cm!(Pinf)$) -- ([yshift=1.28cm]$(P0)!2.0cm!(Pinf)$)
				node [midway, scale=0.5, pos=0.8, black] {$\diagdown\!\!\diagdown$}; %Parallel line indicators
				
			%Kreise bei P_0, P_inf, P_A
			%P_0
			\draw[fill=white](P0) 	circle (1pt)
				node[left, scale=0.8]{$P_0$}
				node[anchor=north west, scale=0.8, inner xsep=0.05cm, inner ysep=0cm]{$s=0$};
			%P_inf
			\draw[fill=white](Pinf) circle (1pt)
				node[right, scale=0.8]{$P_\infty$, $s=\infty$};
			%P_A
			\draw[fill=white](PA) 	circle (1pt)
				node[anchor=south west, scale=0.8]{$P_A$, $s=1$};
				
			% Beschriftung von Bremse 
			\node[right, scale=0.8] at (Ossanna.39) {Bremse};
			
			% Beschriftung von Generator 
			\node[right, scale=0.8] at (0,-1.2cm) {Generator};
			
			%Strompfeile
			% Werden mit Hilfe der Tangente gezeichnet.
			% I1,N,G
			\draw[Zeiger,red] (Ursprung) coordinate (a) -- (tangent cs:node=Ossanna, point={(a)}, solution=1)
				node[pos=0.8, scale=0.8, left]{$\hat{\underline{i}}_{1,N,G}$}
				node[HelpNode](Ipng){};
			% I1,N
			\draw[Zeiger,red] (Ursprung) coordinate (a) -- (tangent cs:node=Ossanna, point={(a)}, solution=2)
				node[pos=0.8, scale=0.8, left]{$\hat{\underline{i}}_{1,N}$}
				node[pos=1.1, scale=0.8, left, black]{$P_N$}
				node[HelpNode](Ipn){};
			
			%Spannungszeiger 
			\draw[Zeiger, blue] (Ursprung) -- (90:\USpitze)
				node[pos=1.0, right] {$\hat{\underline{u}}_1$}
				node[pos=0.8, right, black, scale=0.8] {Motor}
				node[HelpNode](U1){};
				
			% Hilfspfade zur Bestimmung der Schnittpunkte
			% von Generatorstrom und Linien
			\path[name path=helppathG] (Ipng) -- (Ipng |- 0,2);
			% von Motorstrom und Linien
			\path[name path=helppathM] (Ipn)  -- (Ipn |- 0,-1);
			% von Punkt P_A und Linien
			\path[name path=helppathPA] (PA)  -- (PA |- 0,-1);
			
			%Bestimmen des Schnittpunktes mit der mechanischen Leistungslinie
			\draw [name intersections={of=helppathG and MechLinie, name=interGM}, gray, fill=gray]
				(interGM-1) -- (Ipng)
				(interGM-1) circle (1pt);
			%Bestimmen des Schnittpunktes mit der Drehmomentlinie	
			\draw [name intersections={of=helppathG and DrehLinie, name=interGD}, gray, fill=gray]
				(interGD-1) circle (1pt);
			%Bestimme Schnittpunkt von Paralleler Drehmomentlinie und Ossannakreis
			\draw [name intersections={of=DrehlinieParallel and Ossannakreis, name=interDO}, gray, fill=gray]
				(interDO-1) circle (1pt)
				node[above, black, scale=0.8](PK) {$P_K$};
			%Hilfspfad für Mk Linie
			\path[name path=helppathPK] (PK)  -- (PK |- 0,-1);
			
			%Hilfslinien....
			\draw[gray] (interGM-1) -- ++(0.2,0);
			\draw[gray] (interGD-1) -- ++(0.6,0);
			\draw[gray] (Ipng) 		-- ++(1.0,0);
			
			%P_Mech
			\draw[{Latex[length=1mm]}-{Latex[length=1mm]}, gray] ($(Ipng) + (0.2,0)$) -- ($(interGM-1) + (0.2,0)$)
				node[pos=0.4, below, sloped, scale=0.8]{$P_{mech}$};
			%M_G
			\draw[{Latex[length=1mm]}-{Latex[length=1mm]}, gray] ($(Ipng) + (0.6,0)$) -- ($(interGD-1) + (0.6,0)$)
				node[pos=0.4, below, sloped, scale=0.8]{$M_G$};
			%P_el
			\draw[{Latex[length=1mm]}-{Latex[length=1mm]}, gray] ($(Ipng) + (1.0,0)$) -- ($(Ipng |- 0,0) + (1.0,0)$)
				node[pos=0.5, below, sloped, scale=0.8]{$P_{el}$};			
			%M_N
			\draw [name intersections={of=helppathM and DrehLinie, name=interMD}, gray]
				(interMD-1) -- (Ipn)
				node[pos=0.6, scale=0.8, right]{$M_N$};
			%M_A
			\draw [name intersections={of=helppathPA and DrehLinie, name=interPAD}, gray]
				(interPAD-1) -- (PA)
				node[pos=0.35, scale=0.8, left]{$M_A$};
			%M_K
			\draw [name intersections={of=helppathPK and DrehLinie, name=interPKD}, gray]
				(interPKD-1) -- (PK)
				node[pos=0.6, scale=0.8, left]{$M_K$};
			
			%Winkelbeschriftung phi
			\path (Ipn) -- (Ursprung) -- (U1)
				pic[Winkelpfeil, angle radius=6.0mm, pic text=$\varphi$, angle eccentricity=0.7] {angle=Ipn--Ursprung--U1};
		\end{tikzpicture}
		%----------------------------------------------------------------------------------------------------------------%	
	\end{flushright}	
\end{frame}
...
\begin{frame}\frametitle{Überlagerung sinusförmiger Schwingungen}	
	\begin{flushright}
		...
		\begin{tikzpicture}
			...
			\coordinate (xEnd) 	   at (3.3,0);  	%Endpunkt der x-Achse (Im-Achse)
...
\begin{frame}\frametitle{Fourierreihen - Spektralform}	
	\begin{flushright}
		%-----------------------------------------------------------------------------------------------------------------%
		\tikzsetnextfilename{FourierreihenSpektralform_a}
		\begin{tikzpicture}
			%Verschiedene Styles definieren:
			[
			HelpNode/.style={at end, inner sep=0pt, outer sep=0pt, minimum size=0pt},  %Options für Hilfs-Node (z.B. am Ende eines Zeigers)
			Zeiger/.style={-{Latex[length=2mm]}, blue, thick},					   	   %Style der komplexen Zeiger
			Winkelpfeil/.style={-{Latex[length=1mm]}, draw, pic text=$\varphi_n$,
				 angle radius=4mm, angle eccentricity=1.4}, 						   %Style des Winkelpfeils	 
			HelpLine/.style={densely dashed, very thin, gray}						   %Hilfslinien
			]
			% Verschiedene Variabeln definieren:
			\def\An{1.0};		%Länge von a_n
			\def\Bn{1.2};		%Länge von b_n
			...
			% Plotten der Zeiger
			%a_n: 
			\draw[Zeiger] (Ursprung) -- ++(0:\An)
				node[pos=1.0, right] {$\hat{a}_n$}
				node(tmpAn) [HelpNode]{};
			%b_n: 
			\draw[Zeiger] (Ursprung) -- ++(-90:\Bn)
				node[pos=0.7, left] {$\hat{b}_n$}
				node(tmpBn) [HelpNode]{};
			%c_n:
			\draw[Zeiger, black] (Ursprung) -- ++($(0:\An) + (-90:\Bn)$)
				node[pos=0.95, anchor=north west] {$\hat{c}_n$}
				node(tmpCn) [HelpNode]{};
			
			%% Hilfslinien %%
			\draw[HelpLine] (tmpBn) -- (tmpCn) -- (tmpAn); 

			%Winkel 
			%Phi_n:
			\path (tmpBn) -- (Ursprung) -- (tmpCn)
				pic[Winkelpfeil] {angle=tmpBn--Ursprung--tmpCn};
				
			%Drehrichtungspfeil
			\draw[-{Latex[length=1.5mm]}] (-45:\An+0.5) 
				arc[start angle=-45, delta angle=35, x radius=\An+0.2, y radius=\An+0.2]
				node[pos=0.2, right]{$n\omega$}; 
		\end{tikzpicture}
		%-----------------------------------------------------------------------------------------------------------------&
	\end{flushright}	
\end{frame}
...
\begin{frame}\frametitle{Ergänzung: Periodizität der Tangens-Funktion}	
	\begin{flushright}
		%-----------------------------------------------------------------------------------------------------------------%
		\tikzsetnextfilename{ErgaenzungPeriodizitaetDerTangensFunktion}
		\begin{tikzpicture}
			%Verschiedene Styles definieren:
			[
			HelpNode/.style={at end, inner sep=0pt, outer sep=0pt, minimum size=0pt},  %Options für Hilfs-Node (z.B. am Ende eines Zeigers)
			Zeiger/.style={-{Latex[length=2mm]}, blue, thick},					   	   %Style der komplexen Zeiger
			Winkelpfeil/.style={-{Latex[length=1mm]}, draw, pic text=$\varphi$,
				angle radius=8mm}, 						  	   						   %Style des Winkelpfeils	 
			HelpLine/.style={densely dashed, very thin, gray},						   %Hilfslinien
			Achsenpfeil/.style={->},												   %Style des Achsenpfeils 
			]
			% Verschiedene Variabeln definieren:
			\def\Zlen{1.5};		%Länge von Z
			\def\PhiZ{45};		%Winkel von Z
			
			% Definieren von verschiedenen Koordinaten
			\coordinate (Ursprung) at (0,0);		%Definieren des Ursprungs
			\coordinate (xStart)   at (-0.1,0);		%Startpunkt der x-Achse (Re-Achse)
			\coordinate (xEnd) 	   at (2.3,0);  	%Endpunkt der x-Achse (Im-Achse)
			\coordinate (yStart)   at (0,-0.1);  	%Startpunkt der y-Achse
			\coordinate (yEnd) 	   at (0,1.5);  	%Endpunkt der y-Achse 

			%%%%%%%%%%%%%%%%%%%%%%%%%% Zeigerdiagramm %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			
			% Achsen 
			\draw[Achsenpfeil] (xStart) -- (xEnd) node[at end, below]{$X$}; %x-Achse 
			\draw[Achsenpfeil] (yStart) -- (yEnd) node[at end, left] {$jY$};%y-Achse
			\node[anchor=north east] at (Ursprung) {$0$}; 					%Beschriftnug des Urpsrungs 
			
			% Plotten der Zeiger 
			%Z
			\draw[Zeiger] (Ursprung) --++ (\PhiZ:\Zlen)
				node[near end, left] {$\lvert\underline{Z}\rvert\textcolor{black}{=Z}$}
				node[anchor=south west, scale=0.8, black] {$\textcolor{blue}{\underline{Z}}=X+jY$}
				node(tmpZ) [HelpNode]{}; 
			% Winkel-Beschriftung
			\path (xEnd) -- (Ursprung) -- (tmpZ) 
				pic[Winkelpfeil] {angle = xEnd--Ursprung--tmpZ};%Winkelpfiel	
			% Hilfslinien
			% waagerecht
			\draw[HelpLine] (Ursprung |- tmpZ) -- (tmpZ)
				node[at start, anchor=east, scale=0.8, black] {$Y=\operatorname{Im}\{\underline{Z}\}$};	
			% senkrecht
			\draw[HelpLine] (Ursprung -| tmpZ) -- (tmpZ)
				node[at start, anchor=south west, scale=0.8, black] {$X=\operatorname{Re}\{\underline{Z}\}$};
		\end{tikzpicture}
		%-----------------------------------------------------------------------------------------------------------------&
	\end{flushright}	
\end{frame}
...
\begin{frame}\frametitle{RL-Schaltung mit gleichgerichteter Spannung II}	
	\begin{flushright}
		%-----------------------------------------------------------------------------------------------------------------%
		\tikzsetnextfilename{RLSchaltungMitGleichgerichteterSpannung_II_c}
		\begin{tikzpicture}
			%Verschiedene Styles definieren:
			[
			HelpNode/.style={at end, inner sep=0pt, outer sep=0pt, minimum size=0pt},  %Options für Hilfs-Node (z.B. am Ende eines Zeigers)
			Zeiger/.style={-{Latex[length=2mm]}, blue, thick},					   	   %Style der komplexen Zeiger
			Winkelpfeil/.style={-{Latex[length=1mm]}, draw, pic text=$\varphi_{ui}$,
				angle radius=4mm, angle eccentricity=1.6}, 						  	   %Style des Winkelpfeils	 
			]
			% Verschiedene Variabeln definieren:
			\def\USpitzeR{0.8};		%Länge von U_r
			\def\USpitzeL{1.2};		%Länge von U_L
			\def\ISpitze{0.5};		%Länge von I
			...
			% Plotten der Zeiger 
			%U_R
			\draw[Zeiger] (Ursprung) --++ (0:\USpitzeR)
				node[pos=1.3, below] {$\underline{\hat{u}}_R=R\textcolor{red}{\underline{\hat{i}}}$}
				node(tmpUR) [HelpNode]{}; 
			%U_R
			\draw[Zeiger] (tmpUR) --++ (90:\USpitzeL)
				node[pos=0.5, right] {$\underline{\hat{u}}_L=j\omega L\textcolor{red}{\underline{\hat{i}}}$}
				node(tmpUL) [HelpNode]{};
			%U_G
			\draw[Zeiger] (Ursprung) -- (tmpUL)
				node[pos=0.6, anchor=south east] {$\underline{\hat{u}}_G=\underline{Z}\textcolor{red}{\underline{\hat{i}}}$}
				node(tmpUG) [HelpNode]{}; 
			%I
			\draw[Zeiger, red] ($(Ursprung) - (0,0.1cm)$) --++ (0:\ISpitze)
				node[pos=0.5, below] {$\underline{\hat{i}}$}
				node(tmpI) [HelpNode]{}; 
			% Winkel-Beschriftung
			\path (tmpUR) -- (Ursprung) -- (tmpUL) 
				pic[Winkelpfeil] {angle = tmpUR--Ursprung--tmpUL};%Winkelpfiel	
		\end{tikzpicture}
		%-----------------------------------------------------------------------------------------------------------------&
	\end{flushright}	
\end{frame}
...
\begin{frame}\frametitle{Clicker - RLC Serienschaltung Zeiger a,b,c}	
	\begin{flushright}
		%-----------------------------------------------------------------------------------------------------------------%
		\tikzsetnextfilename{Clicker_RLC_SerienschaltungZeiger_a_b_c}
		\begin{tikzpicture}[scale=0.7]
			%%% Definitionen des Settings %%%
			\pgfkeys{/pgf/fpu=true,/pgf/fpu/output format=fixed}
			\def\Alpha{0}
			...
			%Positionen der Plots definieren
			\def\Xstep{4.0} %Schritt in x-Richtung bis zum Nächsten Plot
			
			\def\Xoffa{0} 							%X-Position für a
			\pgfmathsetmacro{\Xoffb}{\Xoffa+\Xstep} %X-Position für b
			\pgfmathsetmacro{\Xoffc}{\Xoffb+\Xstep} %X-Position für c
			\def\Yoff{0} 							%Y-Position für a,b,c
			...
			%Doku zu \foreach: pgfmanual.pdf Kapitel 87, Abschnitt "multiple variables" (Seite 974) 
			\foreach \Xoff/\fre/\LabPosR/\LabPosQ/\Bool/\Label in
			{\Xoffa/\frea/below/above/0/a, \Xoffb/\freb/above/below/0/b, \Xoffc/\frec/below/above/1/c}
			{
				%%%% Rechnung %%%% 
				\pgfkeys{/pgf/fpu=true,/pgf/fpu/output format=fixed}
				\pgfmathsetmacro{\w}{ 2*pi*\fre}
				...
				% Widerstandsspannung
				\draw[-{Latex[length=2mm]}, color=\FarbeSpannung,  thick] 
				(\Alpha:0)++(\Xoff,\Yoff) -- ++(\Alpha:\uR) node [midway, \LabPosR] {$\mzeiger{u}_R\;$}
				\if\Bool1{
					node [midway, \LabPosQ] {$\mzeiger{u}\;$}
				}\fi
				node[at end, inner sep=0pt, outer sep=0pt, minimum size=0pt](tmp) {};
				...
				\if\Bool0{
					\draw[-{Latex[length=2mm]}, color=\FarbeSpannung,  thick] 
					(\Xoff,\Yoff) -- ++(tmp2) node [midway, \LabPosQ] {$\;\mzeiger{u}$};
				}\fi
			}
		\end{tikzpicture}
		%-----------------------------------------------------------------------------------------------------------------%	
...
\begin{frame}\frametitle{Clicker - Zeigerdiagramm L/C}	
	\begin{flushright}
		%-----------------------------------------------------------------------------------------------------------------%
		\tikzsetnextfilename{Clicker_Zeigerdiagramm_LC}
		\begin{tikzpicture}
			%Verschiedene Styles definieren:
			[
			Zeiger/.style={-{Latex[length=2mm]}, blue, thick},					   	   %Style der komplexen Zeiger	 
			...
			% Verschiedene Variabeln definieren:
			\def\USpitzeL{1.4};		%Länge von U_L
			\def\USpitzeC{1.8};		%Länge von U_C
			\def\ISpitzeL{1.2};		%Länge von I_L
			\def\ISpitzeC{0.7};		%Länge von I_C
			\def\Phi{35};			%Winkel zwischen I_L und U_C
			
			% Definieren von verschiedenen Koordinaten
			\coordinate (xStart)   at (-0.1,0); 	%Startpunkt der x-Achse (Re-Achse)
			\coordinate (xEnd) 	   at (2.0,0);  	%Endpunkt der x-Achse (Im-Achse)
			\coordinate (yStart)   at (0,-0.1);  	%Startpunkt der y-Achse
			\coordinate (yEnd) 	   at (0,1.5);  	%Endpunkt der y-Achse 
			\coordinate (Ursprung) at (0,0);		%Definieren des Ursprungs
			...
			% Plotten der Zeiger 
			%U_C
			\draw[Zeiger] (Ursprung) --++ (0:\USpitzeC)
				node[pos=0.6, below] {$\underline{\hat{u}}_C$};
			%U_L
			\draw[Zeiger] (Ursprung) --++ (\Phi+90:\USpitzeL)
				node[pos=0.5, left] {$\underline{\hat{u}}_L$};
			%I_C
			\draw[Zeiger, red] (Ursprung) -- (90:\ISpitzeC)
				node[pos=0.7, right] {$\underline{\hat{i}}_C$};
			%I_L
			\draw[Zeiger, red] (Ursprung) -- (\Phi:\ISpitzeL)
				node[at end, right] {$\underline{\hat{i}}_L$};
		\end{tikzpicture}
		%-----------------------------------------------------------------------------------------------------------------&
	\end{flushright}	
\end{frame}
...
\begin{frame}\frametitle{Clicker - Zeigerdiagramm}	
	\begin{flushright}
		%-----------------------------------------------------------------------------------------------------------------%
		\tikzsetnextfilename{Clicker_Zeigerdiagramm}
		\begin{tikzpicture}
			%Verschiedene Styles definieren:
			[		
			HelpNode/.style={at end, inner sep=0pt, outer sep=0pt, minimum size=0pt},  %Options für Hilfs-Node (z.B. am Ende eines Zeigers)
			...
			% Verschiedene Variabeln definieren:
			\def\Zeiger{1.4};		%Länge der Zeiger
			\def\Phi{45};			%Winkel zwischen x-Achse und Zeiger 
			
			% Definieren von verschiedenen Koordinaten
			\coordinate (xStart)   at (-1.0,0); 	%Startpunkt der x-Achse (Re-Achse)
			\coordinate (xEnd) 	   at (1.5,0);  	%Endpunkt der x-Achse (Im-Achse)
			\coordinate (yStart)   at (0,-1.5);  	%Startpunkt der y-Achse
			\coordinate (yEnd) 	   at (0,1.5);  	%Endpunkt der y-Achse 
			\coordinate (Ursprung) at (0,0);		%Definieren des Ursprungs
			...
			% Plotten der Zeiger 
			%Zeiger blau
			\draw[Zeiger, blue] (Ursprung) --++ (\Phi:\Zeiger)
				node(tmp1) [HelpNode]{};
			%Zeiger rot
			\draw[Zeiger, red]  (Ursprung) --++ (-\Phi:\Zeiger)
				node(tmp2) [HelpNode]{};
				
			% Hilfslinien 
			\draw[densely dashed] (tmp1) -- (tmp2);
			\draw[densely dashed] (tmp1 -| Ursprung) -- (tmp1);
			\draw[densely dashed] (tmp2 -| Ursprung) -- (tmp2);
