...
\usepackage{tikz}
\usetikzlibrary{angles, calc, positioning, intersections, topaths, decorations}
...
\begin{frame}\frametitle{Erzeugung Drehfeld III}	
	\begin{flushright}
		...
		\begin{tikzpicture}
			...
			% Verschiedene Variabeln definieren:
			%Spannungen:
			\def\BSpitze{0.7};		%Spitzenwert von Zeiger
			...
			\foreach \Phi/\Xpos in {\PhiE/0, \PhiZ/1, \PhiD/2, \PhiV/3, \PhiF/4}
				{
					...
					\draw[Zeiger] (\Xpos,0) -- ++(\PhiSE:\BE);
					\draw[Zeiger] (\Xpos,0) -- ++(\PhiSZ:\BZ);
					\draw[Zeiger] (\Xpos,0) -- ++(\PhiSD:\BD); 
				}
			
			%\draw[Zeiger] (Ursprung) -- ++(0:-1cm);
%			% Achsen 
%			\draw[Achsenpfeil] (xStart) -- (xEnd); %x-Achse 
%			\draw[Achsenpfeil] (yStart) -- (yEnd); % y-Achse
%			
