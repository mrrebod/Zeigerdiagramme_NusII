...
\begin{frame}\frametitle{Clicker - RLC Serienschaltung Zeiger a,b,c}	
	\begin{flushright}
		...
		\begin{tikzpicture}[scale=0.7]
			...
			%Doku zu \foreach: pgfmanual.pdf Kapitel 87, Abschnitt "multiple variables" (Seite 974) 
			\foreach \Xoff/\fre/\LabPosR/\LabPosQ/\Bool in
			{\Xoffa/\frea/above/below/0, \Xoffb/\freb/above/below/1, \Xoffc/\frec/below/above/0}
			{
				%%%% Rechnung %%%% 
				\pgfkeys{/pgf/fpu=true,/pgf/fpu/output format=fixed}
				\pgfmathsetmacro{\w}{ 2*pi*\fre}
				...
				\pgfkeys{/pgf/fpu=false}
				%% Achsen, Referenzpfeil und Caption %%
				\draw[->] (\Xoff-0.3,\Yoff) -- (\Xoff+1.8,\Yoff) node[anchor= north west] {Re}; % X-Achse
				\draw[->] (\Xoff,\Yoff-0.5) -- (\Xoff,\Yoff+2.5) node[anchor=south east] {Im}; % Y-Achse 
				%Referenzpfeil
				\draw[-{Latex[length=2mm]}, color=\FarbeStrom,  thick]
				(\Xoff, \Yoff-\RefStep) -- (\Xoff+1.5,\Yoff-\RefStep) node[anchor= north] {$\mzeiger{i}\;$};
